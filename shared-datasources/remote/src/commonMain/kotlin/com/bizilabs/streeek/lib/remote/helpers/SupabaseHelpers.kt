...
fun createSupabase(): SupabaseClient =
    createSupabaseClient(
        supabaseUrl = StreeekRemoteConfig.SupabaseUrl,
        supabaseKey = StreeekRemoteConfig.SupabaseKey,
    ) {
        install(Auth)
        install(Storage)
        install(Realtime)
...
private fun String?.toValidErrorMessage(default: String): String {
    return when {
        ...
        this.contains(StreeekRemoteConfig.SupabaseUrl) -> this.replace(StreeekRemoteConfig.SupabaseUrl, "********")
        else -> this
    }
}
