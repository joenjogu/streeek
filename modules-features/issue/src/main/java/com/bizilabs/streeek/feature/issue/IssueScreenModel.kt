...
class IssueScreenModel(
    ...
    private fun getLabels(forceFetch: Boolean) {
        screenModelScope.launch {
            ...
            val update = when (val result = labelRepository.getLabels(page = 1)) {
        ...
        screenModelScope.launch {
            ...
            val update =
                when (val result = issueRepository.createIssue(createIssueDomain = issue)) {
                    is DataResult.Error -> DialogState.Error(
                        title = "Error",
                    ...
                    is DataResult.Success -> {
                        ...
                        mutableState.update { it.copy(id = id) }
                        getIssue()
                        DialogState.Success(
                            title = "Success",
    ...
    fun onValueChangeId(id: Long?) {
        if (state.value.hasLoadedFromNavigation.not())
            mutableState.update { it.copy(id = id, hasLoadedFromNavigation = true) }
    }
