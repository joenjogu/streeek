...
fun String.asDate(format: String = DateFormats.ISO_8601_Z): Instant? {
    return tryOrNull {
        ...
        Instant.parse(this, formatter).let {
            Napier.d("Trying to parse String to Instant :\nVALUE = $this\nFORMAT = $format\nOUTPUT = $it")
            it
        }
    }
}
...
fun String.asLocalDate(format: String = DateFormats.YYYY_MM_DD): LocalDate? {
    Napier.d("Trying to parse String to LocalDate :\nVALUE = $this\nFORMAT= $format")
    return tryOrNull {
...
fun LocalDateTime.isPastDue(): Boolean {
    val inception =
        LocalDate(year = 2025, monthNumber = 1, dayOfMonth = 1).asLocalDateTime().datetimeUTC
